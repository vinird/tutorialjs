var Tutorial={selector:'.tutorial',startIndex:0,endIndex:999,onlyOnce:!1,styles:!0,bodyScroll:!1,resolve:null,reject:null,removeAnimationConflicts:!1,btnFramework:'semantic',btnFinishText:'Terminar',btnFinishClass:'ui button tiny basic',btnNextText:'Siguiente',btnNextClass:'ui button tiny primary',tutorialCount:-999,scrollSelector:null,start:()=>{Tutorial.tutorialCount=Tutorial.startIndex;if(Tutorial.removeAnimationConflicts){$('.animated').css('animation','unset')}
...
if(check){Tutorial.resolve('outOfIndex');return!0}else{Tutorial.setCookie(path,!0,999)}}else{Tutorial.setCookie(path,'',999)}
...
Tutorial.tutorialCount++},checkEndIndex:()=>{if(Tutorial.endIndex==Tutorial.tutorialCount){Tutorial.sanitizeZindex();Tutorial.tutorialCount=-999;Tutorial.resolve("endIndex");Tutorial.hideDimmer();$(Tutorial.selector+'[ tutorial-index='+Tutorial.tutorialCount+']').popup('hide')}},checkStyles:()=>{if(Tutorial.styles){let previusContainer=$(Tutorial.selector+'[ tutorial-index='+(Tutorial.tutorialCount-1)+']');previusContainer.css({'position':'auto',})
let container=$(Tutorial.selector+'[ tutorial-index='+Tutorial.tutorialCount+']');container.css({'position':'relative',})}},createDimmer:()=>{$(document).ready(()=>{if($('.tutorial_dimmer')[0]==undefined){let element='<div style="margin: 0px; padding: 0px; background: #000; opacity: 0; height: 100%; width: 100%; z-index: 999; top: -100%; position: fixed; overflow: hidden;" class="tutorial_dimmer"></div>';$("html").append(element)}})},renderHtmlTutorial:(count)=>{Tutorial.sanitizeZindex();let title=$(Tutorial.selector+'[ tutorial-index='+count+']').attr('tutorial-title')
let text=$(Tutorial.selector+'[ tutorial-index='+count+']').attr('tutorial-text')
return ''+'<strong>'+title+'</strong>'+'<p> '+text+'</p>'+'<button class="'+Tutorial.btnFinishClass+'" tutotrial-val="'+Tutorial.selector+'[ tutorial-index='+count+']'+'" count="'+count+'" onclick="Tutorial.finishTutorial(this)">'+Tutorial.btnFinishText+'</button>'+'<button tutotrial-val="'+Tutorial.selector+'[ tutorial-index='+count+']'+'" class="'+Tutorial.btnNextClass+'" onclick="Tutorial.closePopup(this, '+count+')">'+Tutorial.btnNextText+'</button>'},sanitizeZindex:(finish=!1)=>{$(Tutorial.selector+'[ tutorial-index='+(Tutorial.tutorialCount-1)+']').css('z-index','auto')
if(!finish){$(Tutorial.selector+'[ tutorial-index='+Tutorial.tutorialCount+']').css('z-index','1000')}},closePopup:(element,count)=>{let oldPopup=$(element).attr('tutotrial-val');if($(Tutorial.selector+'[ tutorial-index='+(count+1)+']').offset()!=undefined){let element=$(Tutorial.selector+'[ tutorial-index='+(count+1)+']');let scroll=Tutorial.calcPopUpPosition(element);$(Tutorial.scrollSelector).animate({scrollTop:parseInt(scroll)},"slow",()=>{$(oldPopup).popup('hide')})}else{$(oldPopup).popup('hide')}},calcPopUpPosition:(element)=>{let vh=document.documentElement.clientHeight;vh=vh/3;let position=element.offset().top;return position-vh},finishTutorial:(element)=>{Tutorial.resolve('cancelled')
let popupName=$(element).attr('tutotrial-val');$(popupName).popup('hide')
Tutorial.sanitizeZindex(!0);Tutorial.tutorialCount=-999},showDimmer:()=>{$('.tutorial_dimmer').css('top','0');$('.tutorial_dimmer').animate({opacity:0.4})},hideDimmer:()=>{$('.tutorial_dimmer').animate({opacity:0},()=>{$('.tutorial_dimmer').css('top','-100%')})},setCookie:(cname,cvalue,exdays)=>{var d=new Date();d.setTime(d.getTime()+(exdays*24*60*60*1000));var expires="expires="+d.toUTCString();document.cookie=cname+"="+cvalue+";"+expires+";path=/"},getCookie:(cname)=>{var name=cname+"=";var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++){var c=ca[i];while(c.charAt(0)==' '){c=c.substring(1)}
